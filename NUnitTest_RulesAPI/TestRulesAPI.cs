...
namespace NUnitTest_RulesAPI
{
    public class TestRulesAPI
    {
        ...
        {
            var result = _rulesController.EvaluateMinBal(1, 1400) as OkObjectResult;
            var model = result.Value as RuleStatus;
            Assert.AreEqual(model.Status,"allowed");
        }

        [Test]
        public void EvaluateMinBal_InValidInput_int()
        {
            var result = _rulesController.EvaluateMinBal(2, 500) as OkObjectResult;
            var model = result.Value as RuleStatus;
            Assert.AreEqual(model.Status, "denied");
        }
